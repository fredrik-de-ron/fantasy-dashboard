...
for lag_namn, gw_dict in fdr_per_lag.items():
    ...
    for gw in sorted(gw_dict.keys()):
        ...
        dgw = len(matcher) > 1
        if dgw:
            for i, match in enumerate(matcher):
                resultat[lag_namn].append({
                    "gw":        gw,
                    "dgw":       True,
                    "dgw_match": i + 1,
                    "dgw_total": len(matcher),
                    "fdr":       match["fdr"],
                    "mot":       match["mot"],
                    "hemma":     match["hemma"],
                })
...
for lag_namn, matcher in resultat.items():
    ...
    for m in matcher:
        if m.get("dgw"):
            gw_count.setdefault(m["gw"], []).append(m)
    for gw, dgw_matcher in gw_count.items():
        hb_text = " + ".join(f"{m['mot']} ({'H' if m['hemma'] else 'B'})" for m in dgw_matcher)
        print(f"  Omgång {gw}: {lag_namn} — {hb_text}")
        hittade_dgw = True
if not hittade_dgw:
    print("  Inga DGW i kommande omgångar")
